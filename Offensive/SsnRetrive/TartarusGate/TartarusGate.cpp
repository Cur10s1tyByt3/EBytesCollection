...
// ============================================================
//  TartarusGate.cpp
//  Halo's Gate + detection of mid-stub hooks.
//
//  Some EDRs patch the stub AFTER mov r10,rcx producing:
//    4C 8B D1 E9 ...  (mov r10,rcx then JMP)
//  instead of 0xE9 at byte 0. Tartarus catches both positions
//  and falls back to neighbor scanning for either.
//
// ============================================================
//
...
)
{
    ...
    for (DWORD i = 0; i < Count && Table->Count < MAX_SYSCALLS; i++)
    {
        ...
        // Hook at byte 0.
        //
        CONST BOOL HookedAtStart = (Stub[0] == 0xE9);

        //
        // Hook at byte 3 — lets mov r10,rcx execute then JMPs.
        //
        CONST BOOL HookedAfterMov = (Stub[0] == 0x4C &&
            Stub[1] == 0x8B &&
            Stub[2] == 0xD1 &&
            Stub[3] == 0xE9);

        if (HookedAtStart || HookedAfterMov)
        {
            Found = ScanNeighbors(
...
INT main(VOID)
{
    ...
    {
        printf("[-] Tartarus' Gate failed\n");
        return 1;
    }

    printf("[+] Tartarus' Gate - %d syscalls\n\n", Table.Count);
