...
// ============================================================
//  HellsGate.cpp
//  SSN retrieval by reading stub bytes directly from ntdll.
//
//  Pattern scanned: 4C 8B D1 B8 [lo] [hi] 00 00
//    mov r10, rcx
//    mov eax, <SSN>
//
//  Fails if the stub is hooked with a JMP (0xE9) by an EDR.
// ============================================================
//
...
// ============================================================
//  GetNtdllExports
//  Resolves EAT pointers from the already-loaded ntdll.
//  ntdll is always mapped before any user code runs —
//  GetModuleHandleA cannot return NULL here.
// ============================================================
//
...
// ============================================================
//  HellsGate_GetSsn
//  Reads the SSN from a single unhooked syscall stub.
// ============================================================
//
...
)
{
    ...
    for (INT Offset = 0; Offset < 32; Offset++)
    {
        ...
        if (Stub[Offset] == 0x0F &&
            Stub[Offset + 1] == 0x05) return FALSE;

        if (Stub[Offset] == 0xC3) return FALSE;

        //
        // 4C 8B D1 B8 [lo] [hi] 00 00
        //
        if (Stub[Offset] == 0x4C &&
            Stub[Offset + 1] == 0x8B &&
            Stub[Offset + 2] == 0xD1 &&
...
// ============================================================
//  HellsGate_DumpAll
//  Walks every Zw* export in ntdll and collects SSNs.
// ============================================================
//
...
)
{
    ...
    for (DWORD i = 0; i < Count && Table->Count < MAX_SYSCALLS; i++)
    {
        ...
        if (HellsGate_GetSsn(FuncAddr, &Ssn) == FALSE) continue;
...
INT main(VOID)
{
    ...
    {
        printf("[-] Hell's Gate failed (stubs may be hooked)\n");
        return 1;
    }

    printf("[+] Hell's Gate - %d syscalls\n\n", Table.Count);
